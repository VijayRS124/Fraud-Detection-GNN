...
class FraudGNNTrainer:
    ...
    def _connect_to_neo4j(self): # ... (no changes)
        try:
    ...
    def fetch_graph_from_neo4j(self): # ... (no changes)
    ...
    def prepare_for_training(self): # ... (no changes)
    ...
    def train_and_evaluate(self):
        # <<< --- MODIFICATION 1: DEFINE SAVE PATHS --- >>>
        ...
        # <<< --- MODIFICATION 2: SAVE THE TRAINED MODEL --- >>>
        ...
        for thresh in thresholds:
            ...
            if f1 > best_f1:
                best_f1 = f1
                best_metrics = {
                    'best_threshold': thresh, 'precision': precision,
                    'recall': recall, 'f1_score': f1
                }

        # <<< --- MODIFICATION 3: SAVE METRICS AND CURVE DATA --- >>>
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig(pr_plot_save_path) # Save the figure
